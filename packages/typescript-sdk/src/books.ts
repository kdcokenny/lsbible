 ...
 *
 * Contains complete chapter and verse counts for validation and reference building.
 */
...
export const BIBLE_STRUCTURE: Record<number, BookInfo> = {
  1: {
    number: 1,
    name: "Genesis",
...
export const BOOK_NAMES: Record<number, string> = Object.fromEntries(
  Object.values(BIBLE_STRUCTURE).map((book) => [book.number, book.name])
);

/** Reverse lookup by name to book number (lowercase keys) */
...
/** Bible sections for search result organization */
export const SECTION_NAMES: Record<number, BibleSection> = {
